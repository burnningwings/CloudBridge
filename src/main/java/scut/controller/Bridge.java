...
public class Bridge {
    ...
    @GetMapping(value = "/bridge/image/{bridgeId}/{imageBaseName}")
    public void getImage(@PathVariable("bridgeId") long bridgeId,
                         @PathVariable("imageBaseName") String imageBaseName,
                         HttpServletResponse response) throws IOException {
        ImageDataWrapper imageData = imageService.loadImage("bridge", bridgeId, imageBaseName);
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @PostMapping(value = "/bridge/image/{bridgeId}")
    public void uploadImage(@PathVariable("bridgeId") long bridgeId,
                            @RequestParam("file") MultipartFile file,
                            @RequestParam("type") String MIMEType) {
        try {
            imageService.saveImage("bridge", bridgeId, MIMEType, file);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @DeleteMapping(value = "/bridge/image/{bridgeId}/{imageBaseName}")
    public void deleteImage(@PathVariable("bridgeId") long bridgeId,
                            @PathVariable("imageBaseName") String imageBaseName) throws IOException {
        imageService.deleteImage("bridge", bridgeId, imageBaseName);
    }
